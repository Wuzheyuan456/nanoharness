...
@dataclass
class Span:
    """一个追踪单元：一次 turn / 一次路由 / 一次工具调用。"""
    trace_id: str              # 同一条用户请求共享，串联整棵树
    span_id: str               # 本 Span 唯一 id
    parent_id: str = ""        # 父 Span id，根 Span 为空
    name: str = ""             # Span 名（如 "turn" / "router.classify"）
    start_ts: float = 0.0
    end_ts: float = 0.0
    attributes: dict[str, Any] = field(default_factory=dict)   # 结构化标签
    events: list[dict[str, Any]] = field(default_factory=list)  # Span 内时间点
...
# ContextVar：记录当前协程链路的活跃 Span id，子 Span 自动继承父 id
# 同 TurnRunner/LaneQueue 的 ContextVar 模式，asyncio Task 级隔离
...
class Tracer:
    """
    轻量追踪器。全局单例，所有 Span 存内存，按 trace_id 索引。
    ...
    def __init__(self) -> None:
        ...
        self._otel_enabled = False                     # 是否桥接真 OTel SDK

    def enable_otel(self) -> None:
        """启用 OTel SDK 桥接（需安装 opentelemetry-sdk）。默认关闭。"""
        try:
    ...
    def start_span(
        ...
        **attrs: Any,
        ...
        """
        创建并进入一个 Span。trace_id 为空时继承当前链路的 trace_id，
        都没有则新建（作为根 Span）。
        """
        parent_id = _CURRENT_SPAN_ID.get()
        if trace_id is None:
            # 没有显式 trace_id，从父 Span 继承，或新建根 trace
    ...
    def build_tree(self, trace_id: str) -> dict[str, Any]:
        """
        把扁平 Span 列表组织成树，供 Gradio 面板树形展示。

        返回根 Span 的嵌套字典：{span, children: [...]}
        """
...
class _SpanContext:
    """async with / with 上下文管理器：进入时设当前 span，退出时记录结束时间。"""
    ...
    # 同步 with 也支持，方便非 async 代码段（如工具函数内部）
