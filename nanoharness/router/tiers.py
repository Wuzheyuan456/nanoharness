...
class Tier(StrEnum):
    T0 = "T0"   # 简单问答：打招呼、查事实、单步计算
    T1 = "T1"   # 中等推理：多步骤任务、需要 1~3 次工具调用
    T2 = "T2"   # 复杂任务：长链推理、代码生成、多轮工具调用
    T3 = "T3"   # 高风险决策：需要 extended thinking、高精度要求
...
class TierConfig:
    ...
    model_id: str
    max_tokens: int
    thinking_budget: int = 0   # >0 时启用 extended thinking（仅 T3）
    description: str = ""


# 默认映射表 — 可通过 nanoharness.toml 覆盖
...
# ─── TierRegistry：支持运行时覆盖 ─────────────────────────────────────────────

class TierRegistry:
    """
    持有当前生效的档位配置，支持从 toml/env 覆盖默认值。
