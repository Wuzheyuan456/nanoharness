...
档位定义：
- T0：简单问答、打招呼、知识性问题、是/否判断，**无需任何工具调用**
- T1：中等推理、**需要 1~3 步工具调用**、常规代码问题、数学计算、信息检索
- T2：复杂任务、长链推理、代码生成/重构、多轮工具调用、复杂 SQL/算法
- T3：高风险决策、安全分析、需要深度思考和风险评估

关键判断标准：
- **T0 vs T1**：如果问题是知识性的（可以直接回答，不需要查询外部数据），是 T0；如果需要调用工具获取实时信息或执行操作，是 T1
- **T2 vs T3**：如果任务是实现/分析（有明确目标），是 T2；如果涉及风险评估/架构决策（需要权衡利弊），是 T3

示例：
- "你好" → T0（简单问候，无需工具）
- "什么是递归" → T0（知识性问题，可直接回答）
- "HTTP 状态码 404 是什么意思" → T0（知识性问题，可直接回答）
- "帮我查北京天气" → T1（需要调用天气 API 获取实时数据）
- "123 乘以 456 等于多少" → T1（需要计算工具）
- "推荐一本 Python 书" → T1（需要检索/推荐工具）
- "帮我写一个 LRU 缓存类" → T2（复杂代码生成，有明确目标）
- "帮我设计一个分布式事务系统" → T2（复杂实现，有明确目标）
- "评估是否应该将单体应用拆分为微服务" → T2（复杂分析，有明确目标）
- "支付系统安全审计" → T3（安全分析，涉及风险评估）
- "评估数据库迁移的风险和收益" → T3（架构决策，需要权衡利弊）
