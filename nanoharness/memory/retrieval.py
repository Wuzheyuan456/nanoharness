...
class RetrievalConfig:
    top_k: int = 5                  # 最终注入 context 的条数
    fts_candidate_limit: int = 20   # FTS5 宽召回数量（top_k 的 4 倍）
    time_decay_days: float = 30.0   # 时间衰减半衰期（天）
    importance_weight: float = 0.4  # importance 分在最终得分中的权重
    recency_weight: float = 0.3     # 时间新鲜度权重
    bm25_weight: float = 0.3        # BM25 相关性权重
    # 低于此分数的记忆不注入（防止噪声）
    min_score_threshold: float = 0.1
    # 最近 N 个 session 摘要总是注入（不参与排序）
...
class MemoryRetriever:
    """
    基于 FTS5 BM25 + 时间衰减 + 重要度的混合检索。

    检索流程：
    1. FTS5 宽召回（取 top_k×4 候选）
    2. 对每个候选计算综合得分：
       score = bm25_weight × bm25归一化 + recency_weight × 时间衰减 + importance_weight × importance
    3. 过滤低分，取 Top-K
    4. 最近 N 个 session 摘要额外注入（不竞争名额）
    ...
    def retrieve_with_sessions(
        ...
        """
        返回 (记忆列表, 会话摘要列表)。
        摘要单独返回，由 MemoryManager 决定如何拼接到 context。
        """
    ...
    def format_for_context(
        ...
        """
        把检索结果拼成注入 system prompt 的文字块。
        格式化为自然语言，方便 LLM 直接理解。
        """
        parts: list[str] = []
    ...
    def _time_decay(self, accessed_at: float, now: float) -> float:
        """
        指数衰减：score = e^(-λ×天数)
        半衰期 = time_decay_days，即经过半衰期后得分衰减到 0.5。
