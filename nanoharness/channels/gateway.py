...
# handler 签名：输入入站信封，返回出站信封（或 None 表示不回复）
...
class SafetyPolicy:
    """安全策略，可热更新。"""
    allowed_senders: set[str] = field(default_factory=set)   # 空=不限制
    blocked_senders: set[str] = field(default_factory=set)
    group_require_mention: bool = True    # 群聊默认要求 @机器人
...
class DedupWindow:
    ...
    用 OrderedDict 实现：超容量时淘汰最久未访问的 envelope_id。
    生产级可换成 Redis SETEX，这里内存版够用且零依赖。
    """
    ...
    def check(self, envelope_id: str) -> DedupResult:
        ...
        if len(self._seen) > self._capacity:
            self._seen.popitem(last=False)   # 淘汰最老
...
class Gateway:
    ...
    用法：
        ...
        await gw.start()
        # 通道收到消息后调用 gw.handle_inbound(envelope)
    """
    ...
    async def handle_inbound(self, envelope: InboundEnvelope) -> ChannelSendResult | None:
        """
        处理一条入站信封，返回发送结果（不回复时返回 None）。

        流水线异常不会向上抛，全部降级为日志——保证单条消息异常不拖垮网关。
        """
        # 1. 去重
