...
class TelegramChannel:
    ...
    不在这里建 bot 实例——外部注入 Bot，便于测试用 fake bot 替换。
    """
    ...
    def __init__(self, bot: Any, bot_username: str = "") -> None:
        ...
        self._dp: Any = None   # Dispatcher，start 时才初始化

    async def start(self) -> None:
        """启动 dispatcher。生产环境这里会 start_polling。"""
        # 通道插件对外只暴露 BaseChannel 协议方法，具体平台启动逻辑放在这里
    ...
    def parse_message(self, message: Any) -> InboundEnvelope:
        """
        aiogram Message → InboundEnvelope。

        用 getattr 取字段而不是类型注解，避免类型耦合太紧，
        也方便测试用 SimpleNamespace 伪造 message。
        """
    ...
    async def send(self, envelope: OutboundEnvelope) -> ChannelSendResult:
        ...
        失败处理：RateLimit/RetryAfter 标记 retryable=True，其他异常标记 FAILED。
        """
        ...
        for chunk in chunks:
            try:
                ...
                # aiogram 的 RetryAfter 异常名含 "RetryAfter"，标记可重试
...
def _split_long_text(text: str, max_len: int) -> list[str]:
    """
    把超长文本切分成不超过 max_len 的块。

    优先在换行处切，其次按硬长度切。这是 send 方法的纯函数，
    拆出来便于单元测试（不需要真的发消息）。
    """
    if len(text) <= max_len:
        return [text]
