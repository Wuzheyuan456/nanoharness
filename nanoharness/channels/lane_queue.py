...
# 全局 session_key → 锁 映射（与 TurnRunner 的 _session_locks 同构）
_lane_locks: dict[str, asyncio.Lock] = {}

# ContextVar：当前协程链路持有的车道锁 id，重入时跳过等待避免死锁
...
class LaneQueue:
    """
    车道队列：按 session_key 串行执行任务，跨 session 并行。
    ...
    async def dispatch(
        ...
        """
        把一个协程工厂投递到 session_key 对应的车道。

        coro_factory 是工厂而不是协程，原因：协程在创建时就被调度，
        传工厂能确保串行时才真正启动协程，避免提前占用资源。
        ...
        if lock_id in owned:
            # 重入：当前协程链路已持有该车道锁（subagent 场景），直接执行
