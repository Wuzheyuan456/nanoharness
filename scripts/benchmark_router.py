...
# 让脚本可直接运行（python scripts/benchmark_router.py）
...
# ─── Benchmark 测试集（人工标注 expected_tier）────────────────────────────────
...
def build_router(real: bool) -> LLMRouter:
    """构建 LLMRouter。real=True 用真实 Anthropic，False 用 mock。"""
    if real:
    ...
    return LLMRouter(
        provider=provider,
        registry=TierRegistry(),   # 默认即 DEFAULT_TIER_CONFIGS，无需传参
...
class _MockClassifyProvider:
    """
    Mock provider：按关键词模拟分类，让脚本无 API key 也能 dry-run。

    只用于验证脚本流程，准确率数据无参考价值。
    真实 benchmark 必须用 AnthropicProvider。
    """
    model_id = "mock-haiku"
...
async def run_benchmark(real: bool) -> dict:
    ...
    correct = 0
    confusion: dict[str, int] = {}   # "T0→T1" 次数
