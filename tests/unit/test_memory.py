...
def make_mock_provider(summary_json: str) -> MagicMock:
    """返回 complete() 固定应答 JSON 的 mock provider。"""
...
class TestMemoryStore:
    ...
    def test_upsert_idempotent_same_id(self):
        """相同 id 重复 upsert 不增加条数，content 被更新。"""
        store = fresh_store()
        e = make_entry("旧内容")
        store.upsert(e)
        updated = MemoryEntry(
    ...
    def test_touch_updates_accessed_at(self):
        """touch() 调用后 accessed_at 变新，access_count 增加。"""
        store = fresh_store()
        ...
        store._conn.commit()
        # FTS 触发器需手动同步（测试里绕过 upsert，直接 INSERT，所以 FTS 已通过触发器同步）
...
class TestMemoryRetriever:
    ...
    def test_format_for_context_structure(self):
        """format_for_context 包含两个区块：近期会话摘要 + 相关记忆。"""
        store = fresh_store()
    ...
    def test_retrieve_below_threshold_filtered(self):
        """得分低于 min_score_threshold 的结果被过滤。"""
        store = fresh_store()
        # 写入一条极旧（60天前）、低 importance 的记忆
        ...
        # 得分 ≈ 0.3×1.0 + 0.3×0.25 + 0.4×0.1 ≈ 0.415，低于 0.5 → 被过滤
...
class TestCompactionHook:
    ...
    def test_extract_structured_content_blocks(self):
        """结构化 content 块（list of dicts）的文字能被正确提取。"""
        store = fresh_store()
        hook = MemoryCompactionHook(store=store, agent_id="agent-test", session_key="sess-1")
        msg = Message(
            role="user",
...
class TestSessionConsolidator:
    def test_consolidate_writes_summary_and_facts(self):
        """LLM 返回合法 JSON 时，摘要写入 sessions 表，事实写入 memories 表。"""
        store = fresh_store()
    ...
    def test_consolidate_preference_type(self):
        """type=preference 的 fact 被存为 PREFERENCE 类型。"""
        store = fresh_store()
        provider = make_mock_provider(
    ...
    def test_consolidate_llm_failure_fallback(self):
        """LLM 调用失败时，降级为截断对话存为摘要，不写 facts。"""
        store = fresh_store()
        provider = MagicMock()
        ...
        sessions = store.get_recent_sessions("agent-test")
        assert len(sessions) == 1  # 降级摘要还是写入了
        assert store.count("agent-test") == 0  # 无 facts

    def test_consolidate_bad_json_fallback(self):
        """LLM 返回非 JSON 时触发降级，不崩溃。"""
        store = fresh_store()
    ...
    def test_consolidate_empty_history_skip(self):
        """空 history 不触发 LLM 调用，sessions 表不写入。"""
        store = fresh_store()
        provider = MagicMock()
        provider.complete = AsyncMock()
        consolidator = SessionConsolidator(store, provider)
        ctx = make_ctx()  # history 为空
...
class TestMemoryManager:
    ...
    def test_prefetch_skips_when_history_too_short(self):
        """history 条数不足 prefetch_min_turns 时，返回空字符串且不写入 ctx。"""
    ...
    def test_prefetch_injects_context(self):
        """有记忆且 history 足够时，prefetch 把结果写入 ctx.extra_context。"""
        mgr = MemoryManager(MemoryConfig(db_path=":memory:", prefetch_min_turns=1))
        # 先写入记忆（query 用 trigram 匹配，内容和查询词都需要 ≥3 字符）
    ...
    def test_on_compact_saves_high_value_messages(self):
        """on_compact 把 tool_result 消息写入 L3。"""
    ...
    def test_on_compact_disabled(self):
        """enable_compaction_protection=False 时，on_compact 不写入任何记忆。"""
    ...
    def test_flush_triggers_consolidation(self):
        """flush() 调用后 sessions 表有记录（mock provider 返回合法 JSON）。"""
        provider = make_mock_provider(
        ...
        # prefetch 触发 session_start_times 记录
        asyncio.run(mgr.prefetch(ctx, "测试"))
        # flush 调用 consolidate（背后用 create_task，这里改用 consolidate_and_wait）
