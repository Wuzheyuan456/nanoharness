...
def make_provider(final_text: str = "好的，任务完成。", model: str = "mock-T1") -> MagicMock:
    """返回 mock provider，complete/stream 返回固定文字。"""
    resp = LLMResponse(
...
class TestAgentRegistry:
    ...
    def test_register_overwrite_same_id(self):
        ...
        reg.register(make_card("same-id", ["cap-b"]))  # 覆盖
...
class TestOrchestrator:
    def _make_orchestrator(
        ...
        # worker 也用 t1_provider（complete=synthesize_text，stream=worker_text）
        ...
        # 综合阶段单独 mock：让 T1 的 complete 返回 synthesize_text
        worker_provider.complete = AsyncMock(
            side_effect=[
                # 第一次 complete 调用 = decompose（T0）→ 但我们用 T0 provider
                # 所以 T1 provider 的 complete 只被 synthesize 和 worker NanoCore 调用
                LLMResponse(
    ...
    def test_run_produces_result_with_subtasks(self):
        """正常流程：拆解为 1 个子任务，Worker 执行，返回 OrchestratorResult。"""
    ...
    def test_route_builtin_fallback_when_registry_empty(self):
        """Registry 完全空时，用内置 __fallback__ card 不崩溃。"""
    ...
    def test_max_workers_limits_subtask_count(self):
        """max_workers=2 时，超过 2 个子任务会被截断到 2 个。"""
        reg = AgentRegistry()
        reg.register(make_card("g", ["general"]))
        # 拆解返回 4 个子任务
    ...
    def test_workers_run_in_parallel(self):
        ...
        async def slow_stream(*a, **kw):
            ...
            await asyncio.sleep(0.05)   # 模拟 50ms 工作
            resp = LLMResponse(
...
class TestDebateOrchestrator:
    ...
    def test_review_returns_complete_result(self):
        """正常流程：两个 Reviewer + Judge，返回完整 DebateResult。"""
    ...
    def test_reviewer_a_and_b_independent_sessions(self):
        """两个 Reviewer 使用不同 session_key（独立视角保证）。"""
        seen_sessions: list[str] = []

        async def _stream(*a, **kw):
            # 用 system prompt 的前几字 + 调用时间区分不同 call，但我们通过 AgentContext 验证
            resp = LLMResponse(
    ...
    def test_reviewer_bad_json_degraded_gracefully(self):
        """Reviewer 返回非 JSON 时，降级为原始文字作为 issues，不崩溃。"""
        debate = self._make_debate(
    ...
    def test_parse_judge_rule_merge_disagreement(self):
        """Judge 解析失败时，规则合并：一人 approve 一人 reject → reject。"""
