...
def make_dangerous_tool_provider(dangerous_tool_name: str) -> MagicMock:
    """
    模拟被 prompt 注入的 LLM：始终尝试调用危险工具。
    用于测试"LLM 被欺骗后，执行层能否拦截"。
    """
    tool_call = ToolCall(
...
async def test_unregistered_dangerous_tool_not_in_executed():
    ...
    该工具进入 tools_called 但不进入 tools_executed。
    这验证了注册表是执行层的第一道防线。
    """
    ...
    # LLM 确实请求了危险工具（tools_called 中有记录）
    assert "rm_all" in fp.tools_called

    # 但危险工具没有被成功执行（未注册 → ToolResultEvent.success=False）
...
async def test_safety_constraint_passes_when_no_forbidden_tool_executed():
    """
    正常场景：LLM 只调用 'search'，约束中 must_not_execute_tools 包含 'rm_all'。
    由于 rm_all 从未被执行，约束应该通过（不抛异常）。
    """
...
async def test_safety_constraint_catches_forbidden_tool_violation():
    """
    违规场景：某个工具（'dangerous'）被成功执行，而约束禁止它。
    assert_satisfied 必须抛出 AssertionError，且错误信息可读。
    这是对约束框架本身的测试：它能正确检测到安全边界被突破。
    """
    # 手动构造一个"安全边界已被突破"的指纹
    fp = BehaviorFingerprint(
        tools_called={"dangerous"},
        tools_executed={"dangerous"},   # 违规：dangerous 工具成功执行了
        tool_call_count=1,
        final_state="DONE",
    )
...
async def test_prompt_injection_multiple_dangerous_tools_all_blocked():
    """
    注入攻击场景：LLM 被欺骗尝试调用 'exec_system'（危险，未注册）。
    只有 'search' 在注册表中，exec_system 调用在执行层被拦截。
    """
    provider = make_dangerous_tool_provider("exec_system")
    tools, defs = await _make_safe_registry()  # 只注册 'search'，不注册 exec_system
    ...
    # exec_system 可能出现在 tools_called（LLM 的请求），但绝不能在 tools_executed
...
async def test_registered_safe_tool_is_in_executed():
    ...
    应该出现在 tools_executed 中（确认 tools_executed 的逻辑正确）。
    """
    tools, defs = await _make_safe_registry()
    provider = make_safe_tool_provider("search")
    ...
    # search 是安全合法的工具，应该成功执行
    assert "search" in fp.tools_executed

    # 用约束框架验证：search 必须执行，危险工具不得执行
    constraint = BehaviorConstraint(
...
# ─── BehaviorConstraint 单元测试 ──────────────────────────────────────────────

def test_constraint_tool_count_violation_message():
    """工具调用次数超出范围时，AssertionError 信息包含实际次数和范围。"""
...
def test_constraint_must_complete_violation():
    """Agent 未到达 DONE 状态时，must_complete=True 的约束应抛出异常。"""
...
def test_constraint_must_call_tools_superset_violation():
    """must_call_tools 超集约束：实际调用集合不包含指定工具时报错。"""
    fp = BehaviorFingerprint(
        tools_called={"search"},
    ...
    # 约束要求调用 "search" AND "summarize"，但实际只调用了 "search"
